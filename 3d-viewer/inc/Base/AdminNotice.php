 ...
 *
 * Displays upgrade notices, review prompts, and admin-specific
 * CSS styles for the 3D Viewer plugin.
 */
class AdminNotice
{
    /**
     * Register WordPress hooks for admin notices.
     */
    public function register(): void
    {
        add_action('admin_notices', [$this, 'renderUpgradeNotice']);
        add_filter('admin_footer_text', [$this, 'renderFooterText']);
        add_action('admin_head', [$this, 'renderAdminHeadStyles']);
    }

    /**
     * Display upgrade notice on relevant admin pages (free version only).
     */
    public function renderUpgradeNotice(): void
    {
        ...
        $is_model_viewer_page = $screen->base === 'edit' && $screen->post_type === 'bp3d-model-viewer';
        $is_relevant_page = $screen->base === 'bp3d-model-viewer_page_3dviewer-settings'
            || $is_model_viewer_page
            || $screen->base === 'bp3d-model-viewer_page_3d-viewer-visual-editor';

        if (!$is_relevant_page) {
            return;
        }

        $upgrade_url = esc_url(admin_url('/edit.php?post_type=bp3d-model-viewer&page=3d-viewer#/pricing'));
        $support_url = 'https://wordpress.org/support/plugin/3d-viewer';
        $notice_class = $is_model_viewer_page ? 'bp3d_model_viewer' : 'settings';
?>
        <div class="bp3d_upgrade_notice <?php echo esc_attr($notice_class); ?>">
            <div class="flex">
    ...
    /**
     * Customize admin footer text on model viewer pages.
     */
    public function renderFooterText(string $text): string
    {
        if (get_post_type() !== 'bp3d-model-viewer') {
            return $text;
        }

        $review_url = 'https://wordpress.org/plugins/3d-viewer/reviews/?filter=5#new-post';

        return sprintf(
            'If you like <strong>3D Viewer</strong> please leave us a <a href="%s" target="_blank">Review</a>. Your Review is very important to us as it helps us to grow more.',
            $review_url
        );
    }

    /**
     * Hide the affiliation submenu item via CSS.
     */
    public function renderAdminHeadStyles(): void
    {
?>
        <style>
