...
class Product
{
    ...
    public static function getProductAttributes(array $modelData = []): array
    {
        ...
        return [
            ...
            'shadow' => $meta('3d_shadow_intensity', '1', true),
     ...
     */
    private static function getVariantKeys(mixed $product): array
    {
        $variant_keys = [];
    ...
    public function get_3d_model_html(bool $return = true, string $placement = 'block', bool $popup = false): string
    {
        ...
        $preset = \BP3D\Base\Presets::getPresetById((int) $meta('bp_model_template'));
        ...
        ob_start(); ?>
        <div class="modelViewerBlock wooCustomSelector product_<?php echo esc_attr($class); ?> <?php echo esc_attr($active_class); ?>" data-attributes='<?php echo esc_attr(wp_json_encode($finalData)); ?>'></div>
        <?php
